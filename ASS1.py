...
def get_barcode():
    ...
    while prompt != '0':
        prompt = input("Please enter six digit barcode and press enter. Enter 0 when finished. ")
        ...
        if int(prompt) == UPC_SINGLE:
            ...
            subtotal = calculate_subtotal(Q_SINGLES, Q_SMALLS, Q_LARGES)
            total_tax, total, total_rounded = calculate_total_bill(subtotal)
        elif int(prompt) not in [111111, 666666, 242424, 0]:
            print("Invalid input, please enter barcore.")
...
def get_amount_tendered(total: float) -> float:
    """
    cash = 0
